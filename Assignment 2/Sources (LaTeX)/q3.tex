...
\begin{parts}
    ...
    \begin{tabular}{ | p{0.35\textwidth} | p{0.35\textwidth} | p{0.2\textwidth} | p{0.11\textwidth} | }
    \hline
    ...
    \newline
    {\color{red} \textbf{Solution:} \newline
        Each word in a sentence is subjected to 2 operations: SHIFT and REDUCE (RIGHT-ARC or LEFT-ARC operation), so a sentence containing $n$ words will be parsed in $2n$ steps.
    } \\
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        {\color{red} \textbf{Solution:} 
            \alns{
                \bz_i &= \sum^{m}_{k = 0}\bx_k\bW_{ik} + \bb_{1i} \\
                \frac{\partial \bz_i}{\bx_j} &= \bW_{ij} \\
                \frac{\partial \bh_i}{\partial \bx_j} &= \frac{\partial}{\partial \bx_j}\relu(\bz_i) = \sign(\relu(\bz_i))\frac{\partial \bz_i}{\partial \bx_j} = \sign(\bz_i)\bW_{ij} = \Biggl\{\begin{array}{ll}
                    $$\bW_{ij} &\textit{if }\bz_i > 0$$, \\
                    $$0 &\textit{otherwise.}$$
                \end{array}
            }
        }
        ...
        {\color{red} 
            \textbf{Solution:} \\ 
            $$\frac{\partial \hat{\by}_j}{\partial \bl_i} = \Biggl\{
            \begin{array}{ll}
                \hat{\by}_j(1 - \hat{\by}_j) &\textit{if i = j,} \\
                -\hat{\by}_j\hat{\by}_i &\textit{otherwise.}
            \end{array}$$ \\
            \begin{align*}
                \frac{\partial CE(\by, \hat{\by})}{\partial \bl_i} &= \sum_j\frac{\partial CE(\by, \hat{\by})}{\partial \hat{\by}_j}\frac{\partial \hat{\by}_j)}{\partial \bl_i} = -\sum_j\frac{\partial}{\partial \hat{\by}_j}\by_j\log\hat{\by}_j\frac{\partial \hat{\by}_j}{\partial \bl_i} \\
                &= -\sum_j\frac{\by_j}{ \hat{\by}_j}\frac{\partial \hat{\by}_j}{\partial \bl_i} = \sum_j\Biggl\{\begin{array}{ll}
                    \by_j(\hat{\by}_j - 1) &i = j, \\
                    \by_j\hat{\by}_i &i \neq j,
                \end{array}
            \end{align*} \\
            \text{Since only $\by_c$ equals 1 and the rest $\by_j$ are zeros:} \\
            $$\frac{\partial CE(\by, \hat{\by})}{\partial \bl_i} = \hat{\by}_i - \by_i$$            
        }
...
{\color{red}
 \begin{itemize}
    \item \textbf{Syntactic Disambiguation}: POS tags disambiguate the roles of words, particularly for ambiguous words that can serve multiple syntactic functions (e.g., "book" as a noun or verb). By identifying the syntactic category, the parser can make more informed decisions about dependency relations.
    \item \textbf{Constraint on Dependency Relations}: POS tags impose constraints on valid dependency arcs. For instance, a verb is more likely to govern a noun or an adjective than another verb. This reduces the search space of potential dependency edges, making the parsing process more efficient and accurate.
    \item \textbf{Robustness in Unknown Words}: When the parser encounters rare or unknown words, POS tags can still provide useful syntactic cues, allowing the parser to rely on grammatical patterns rather than lexical familiarity alone.
    \item \textbf{Generalization}: POS tags enable the parser to generalize across different syntactic contexts. Since grammatical functions often correlate more with POS tags than with specific lexical items, using tags helps the model capture broader syntactic patterns that are consistent across different sentences.
\end{itemize}}
