...
$y$ is a one-hot vector. That means that a target vector has 1 in the position corresponding to the target word and 0 in all other positions.
\[\mathcal{L}_{CE} = -\sum_{w \in Vocab}\by_w\log(\hat{\by}_w)\]
...
Therefore, $\mathcal{L}_{CE}=-\log(\hat{\by}_o)$ because all the terms except for the target word's value are going to be zero in the summation. \newline}
...
\begin{enumerate}[label=\roman*.]
    \item
    \begin{align*}
        \frac{\partial{\bJ(\bv_c,o,\bU)}}{\partial{\bv_c}} & = -\frac{\partial}{\partial{\bv_c}}\log{\frac{\exp(\bu^T_o\bv_c)}{\sum_{w \in Vocab}\exp(\bu^T_w\bv_c)}} \\
        & = -\frac{\partial}{\partial{\bv_c}}(\log(\exp(\bu^T_o\bv_c))-\log(\sum_{w \in Vocab}\exp(\bu^T_w\bv_c))) \\
        & = -\frac{\partial}{\partial{\bv_c}}(\bu^T_o\bv_c-\log(\sum_{w \in Vocab}\exp(\bu^T_w\bv_c))) \\
        & = -\bu_o + \sum_{w \in Vocab}\frac{\exp(\bu^T_w\bv_c)\bu_w}{\sum_{w \in Vocab}\exp(\bu^T_w\bv_c)} \\
        & = -\bu_o + \sum_{w \in Vocab}p(O=w|C=c)\bu_w \\
        & = -\bu_o + \sum_{w \in Vocab}\hat{\by}_w\bu_w \\
        & = \bU(\hat{\by} - \by)
    \end{align*}
    \item
    This gradient is 0 when $\hat{\by} = \by$. It means that the model didn't make any errors.
    \item 
    The goal of the gradient descent algorithm is to minimize the loss function. By computing the gradient of the loss function we find the direction of our movement. Now, when we update our $\bv_c$ by a small amount in the opposite direction of that gradient, we will be moving towards a value of $\bv_c$ that leads to a lesser output of the loss function. \newline 
\end{enumerate}}
...
{\color{red} \textbf{Solution:} \newline
    L2 normalization can potentially remove useful information when considering the magnitudes of word vectors. Specifically, if two word vectors, say $\bu_x$ and $\bu_y$, are parallel (i.e., $\bu_x = \alpha \bu_y$ for some $\alpha > 0$), L2 normalization will transform both vectors to the same unit length. Consequently, the magnitude information distinguishing $\bu_x$ from $\bu_y$ will be lost. \newline
    However, L2 normalization generally preserves the semantic relationships encoded in the direction of word vectors. Thus, if word vectors represent semantically distinct concepts, normalization would not significantly affect the classification task's performance since the relative directions of the vectors, which are crucial for capturing semantic relationships, remain intact. \newline 
}
...
if $w = o$:
\begin{align*}
    \frac{\partial{\bJ(\bv_c,o,\bU)}}{\partial{\bu_o}} & = -\frac{\partial}{\partial{\bu_o}}\log(\exp(\bu^T_o\bv_c)) + \frac{\partial}{\partial{\bu_o}}\log(\sum_{w \in Vocab}\exp(\bu^T_w\bv_c))) \\
    & = -\bv_c + \frac{\exp(\bu^T_o\bv_c)\bv_c}{\sum_{w \in Vocab}\exp(\bu^T_w\bv_c)} \\
    & = \bv_c(\hat{\by}_w - 1)
\end{align*}
if $w \neq o$:
\begin{align*}
    \frac{\partial{\bJ(\bv_c,o,\bU)}}{\partial{\bu_o}} & = -\frac{\partial}{\partial{\bu_o}}\log(\exp(\bu^T_o\bv_c)) + \frac{\partial}{\partial{\bu_o}}\log(\sum_{w \in Vocab}\exp(\bu^T_w\bv_c))) \\
    & = 0 + \frac{\exp(\bu^T_o\bv_c)\bv_c}{\sum_{w \in Vocab}\exp(\bu^T_w\bv_c)} \\
    & = \bv_c\hat{\by}_w
\end{align*}
}
...
{\color{red} \textbf{Solution:} \newline
We remember, that $\by$ is a one-hot vector, so:
\begin{align*}
    \frac{\partial{\bJ_{naive-softmax}(\bv_c,o,\bU)}}{\partial{\bU}} & = \bv_c(\hat{\by} - \by)^T
\end{align*}
}
